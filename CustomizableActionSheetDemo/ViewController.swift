...
class ViewController: UIViewController {
  ...
  @IBAction func buttonShowWasTapped() {
    ...
    if let sampleView = UINib(nibName: "SampleView", bundle: nil).instantiate(withOwner: self, options: nil)[0] as? SampleView {
      ...
      var sampleViewItem = CustomizableActionSheetItem(type: .view, height: 100)
      sampleViewItem.view = sampleView
      items.append(sampleViewItem)
    }

    // Second button
    var clearItem = CustomizableActionSheetItem(type: .button)
    ...
    // Third button
    var closeItem = CustomizableActionSheetItem(type: .button)
